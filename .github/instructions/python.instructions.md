...
- You may use the following shortcuts
  ...
  # Run linting (with ruff) and type checking (with ty).
...
- The usual `uv run pytest` like standard pytest does not show test output.
  Run individual tests and see output with `uv run pytest -s some/file.py`.

- Always run `uv run prek run --all-files` and `uv run pytest` to check your code after changes.
...
- Be sure to resolve the ty linter errors as you develop and make
  changes.

- If type checker errors are hard to resolve, you may add a comment `# ty: ignore`
  to disable Ty warnings or errors but ONLY if you know they are not a real problem
...
## Guidelines for Dataclasses vs. Pydantic BaseModel

- Use stdlib `@dataclass` for internal, lightweight data containers where types are
    enforced by the type checker, not at runtime.
    - Prefer `slots=True` and `frozen=True` for memory/performance and to avoid accidental
        mutation.
    - Add validation only if absolutely necessary (and then keep it minimal).

- Use Pydantic `BaseModel` for all I/O boundaries (API payloads, config, env, JSON/CSV,
    DB rows) where parsing, coercion, and validation matter.
    - Stick to v2 APIs: `.model_validate(...)` for parsing, `.model_dump()` for
        serialization, `TypeAdapter(...)` for ad-hoc validation.
    - Use `field_validator`/`model_validator` for invariants, and `computed_field` when
        derived values must appear in dumps.
- Keep models small and declarative (business logic lives elsewhere).
- Set strict configs like `model_config = ConfigDict(extra="forbid", frozen=True)` when
    possible.
- Avoid `arbitrary_types_allowed` unless truly needed, and prefer explicit defaults over
    `None`.

- Don't mix pydantic dataclasses unless you have a compelling reason.
    If you must bridge types, convert via `.model_validate(...)` and `.model_dump()`
    rather than ad-hoc dict munging.
