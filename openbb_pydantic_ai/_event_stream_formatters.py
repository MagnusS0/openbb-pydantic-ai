...
def _format_discovery_meta_result(
    ...
    if tool_name in {"list_tools", "search_tools"} and isinstance(content, Mapping):
        ...
        return _tool_list_details(
            _extract_discovery_tool_entries(content),
            label=label,
            count_label=count_label,
        )

    if tool_name in {"list_tools", "search_tools"} and isinstance(content, str):
        label, count_label = _discovery_tool_labels(tool_name)
        return _tool_list_details(
            _parse_markdown_tool_listing(content),
            label=label,
            count_label=count_label,
        )
    ...
    if tool_name == "get_tool_schema" and isinstance(content, str):
        ...
        _add_parameter_details(details, parsed)
    ...
    if tool_name == "call_tools" and isinstance(content, str):
