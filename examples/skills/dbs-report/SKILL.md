...
For the complete formatting specification, see [report-spec.md](references/report-spec.md).
For the HTML/CSS template, see [references/template.md](references/template.md).
...
Use the template from [template.md](references/template.md) and populate all sections. The HTML must be self-contained with inline CSS and embedded SVG charts.
...
Produce a single HTML file named `{CompanyName}_DBS_Report`. The file must:
